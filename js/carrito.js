...
const cartElements = {
  ...
  preview: document.querySelector("#orderMessagePreview"),
  copyButton: document.querySelector("#copyOrderButton"),
  fallback: document.querySelector("#whatsappFallback"),
};
...
function bindCartEvents() {
  ...
  cartElements.copyButton?.addEventListener("click", copyCartMessage);
  cartElements.customerName?.addEventListener("input", updateCartMessagePreview);
  cartElements.customerNotes?.addEventListener("input", updateCartMessagePreview);
  ...
  window.addEventListener("storage", (event) => {
    if (event.key !== CART_STORAGE_KEYS.cart && event.key !== null) return;
...
function pruneMissingCartItems() {
  ...
  saveCart();
  setCartPageMessage("Se quitaron de tu pedido opciones que ya no aparecen en el catálogo. Consultanos por WhatsApp si las necesitás.");
}
...
function sendCartToWhatsApp() {
  ...
  const { message, url } = prepareCartMessage();
  const longMessage = url.length > 12000;
  const destination = longMessage ? `https://wa.me/${CART_CONFIG.whatsappNumber}` : url;
  if (cartElements.fallback) {
    cartElements.fallback.href = destination;
    cartElements.fallback.hidden = false;
  }

  try {
    window.open(destination, "_blank", "noopener,noreferrer");
    setCartPageMessage(longMessage
      ? "Tu pedido es extenso. Copiá la consulta y pegala en WhatsApp para enviarla completa."
      : "Tu consulta está preparada. Revisala y enviala en WhatsApp. Si no se abrió, usá el enlace de arriba.");
  } catch (error) {
...
function prepareCartMessage() {
  const message = buildCartWhatsAppMessage(
    getCartPageItems(),
    cleanCartText(cartElements.customerName?.value, 80),
    cleanCartText(cartElements.customerNotes?.value, 300),
  );
  return { message, url: `https://wa.me/${CART_CONFIG.whatsappNumber}?text=${encodeURIComponent(message)}` };
}

function updateCartMessagePreview() {
  const items = getCartPageItems();
  if (cartElements.preview) cartElements.preview.value = items.length ? prepareCartMessage().message : "Agregá productos para preparar tu consulta.";
  if (cartElements.copyButton) cartElements.copyButton.disabled = items.length === 0;
  if (cartElements.fallback) cartElements.fallback.hidden = true;
}

async function copyCartMessage() {
  if (!getCartPageItems().length) return;
  try {
    await navigator.clipboard.writeText(prepareCartMessage().message);
    setCartPageMessage("Consulta copiada. Podés pegarla en WhatsApp.");
  } catch {
    const details = document.querySelector("#messagePreviewDetails");
    if (details) details.open = true;
    cartElements.preview?.focus();
    cartElements.preview?.select();
    setCartPageMessage("Seleccionamos el mensaje. Usá Copiar en tu dispositivo y pegalo en WhatsApp.");
  }
}
...
function loadCart() {
  try {
    const raw = safeCartStorageGet(CART_STORAGE_KEYS.cart) || "[]";
    if (raw.length > 180000) return [];
